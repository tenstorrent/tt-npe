...
class BlackholeMultichipDeviceModel : public npeDeviceModel {
    public:
     ...
     ~BlackholeMultichipDeviceModel() override = default;
     ...
     nocRoute route(nocType noc_type, const Coord &startpoint, const NocDestination &destination)
         ...
         // route using blackhole model, then swap out device_id for correct one
         nocRoute dev0_route = _blackhole_model.route(noc_type, startpoint, destination);
         return changeRouteDeviceID(dev0_route, startpoint.device_id);
     }
     ...
     void modelCongestion(
         ...
         float LINK_BANDWIDTH = getLinkBandwidth(nocLinkID());
         static auto worker_sink_absorption_rate =
             _blackhole_model.getSinkAbsorptionRateByCoreType(CoreType::WORKER);
     ...
     size_t getCols() const override { return _blackhole_model.getCols(); }
     size_t getNumChips() const override { return _num_chips; }
     const boost::unordered_flat_set<DeviceID> &getDeviceIDs() const override {
         return _device_ids;
     }
     ...
     const std::vector<nocLinkType> &getLinkTypes() const override {
         return _blackhole_model.getLinkTypes();
     }
     ...
     const std::vector<nocNIUType> &getNIUTypes() const override {
         return _blackhole_model.getNIUTypes();
     }
     ...
     // we can directly reuse wormhole_b0 implementations.
     CoreType getCoreType(const Coord &c) const override {
         return _blackhole_model.getCoreType(c);
     }
     BytesPerCycle getSrcInjectionRate(const Coord &c) const override {
         return _blackhole_model.getSrcInjectionRate(c);
     }
     BytesPerCycle getSinkAbsorptionRate(const Coord &c) const override {
    ...
    protected:
     BlackholeDeviceModel _blackhole_model;
     size_t _num_chips = 8;

...
def group_traces_metal(noc_trace_files):
    ...
    for noc_trace_file in noc_trace_files:
        ...
        noc_trace_files_per_device[dev_id].append((op_id, noc_trace_file))
    ...
    for i in range(num_ops):
        ...
        for dev_id in devices:
            ...
            op_id, noc_trace_file = noc_trace_files_per_device[dev_id][i]
            op_trace_files.append(noc_trace_file)
            max_op_id = max(max_op_id, op_id)

        noc_trace_files_per_op[max_op_id] = ("UnknownOP", op_trace_files)

    return noc_trace_files_per_op

# to do: rename op_id to program runtime id and mesh_workload_id to op_id???
# to do: tracy/profile_this should delete root npeviz directory (not the one in the report) before running again, like it does with .logs
# to do: add docs for visualizer before meeting to address usability challenges
def get_ttnn_mesh_workload_id(op_id):
    # op_id (21 bits) | device id (10 bits)
    return op_id >> 10

def group_traces_ttnn(noc_trace_files):
    # extract dev id, opname, and opid from filename and group by mesh_workload_id
    noc_trace_files_per_op = {}
    ...
    for noc_trace_file in noc_trace_files:
        ...
        mesh_workload_id = get_ttnn_mesh_workload_id(op_id)

        if mesh_workload_id not in devices_per_op:
            devices_per_op[mesh_workload_id] = []
        # There cannot be multiple traces for a device in a single op, this likely means there are old traces present too
        if (dev_id in devices_per_op[mesh_workload_id]):
            log_error("There seem to trace files from multiple runs in the trace directory, please remove old traces and run again.")
        devices_per_op[mesh_workload_id].append(dev_id)

        if mesh_workload_id not in noc_trace_files_per_op:
            noc_trace_files_per_op[mesh_workload_id] = (op_name, [])
        
        # check op name matches
        if noc_trace_files_per_op[mesh_workload_id][0] != op_name:
        ...
        noc_trace_files_per_op[mesh_workload_id][1].append(noc_trace_file)
...
def analyze_noc_traces_in_dir(noc_trace_dir, emit_viz_timeline_files, compress_timeline_files=False, group_as_metal_traces = False,
    ...
    for mesh_workload_id, op_name_and_trace_files in noc_trace_files_per_op.items():
        op_name, op_trace_files = op_name_and_trace_files
        output_file_path = os.path.join(noc_trace_dir, f"noc_trace_ID{mesh_workload_id}_merged.json")
        process_traces(
            topology, op_trace_files, output_file_path, remove_desynchronized_events, quiet
        )
        noc_trace_info.append((output_file_path, op_name, mesh_workload_id))

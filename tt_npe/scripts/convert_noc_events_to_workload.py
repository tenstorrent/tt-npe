    ...
    for event in event_data_json:
        ...
        try:
            ...
            if sx == dx and sy == dy:
                ...
                sy != dy and sx != dx
            ):  # diagonal, unaligned transfer has the highest latency
        ...
        transfer["noc_event_type"] = (
            noc_event_type if noc_event_type != "WRITE_" else "WRITE"
        )
...
def get_cli_args():
    ...
    parser.add_argument(
        "input_filepath", type=str, help="Path to the JSON file to load"
    )
    parser.add_argument(
        "output_filepath", type=str, help="Path to the JSON file to load"
    )
    parser.add_argument("--quiet", action="store_true", help="Silence stdout")
...
def main():
    ...
    convert_noc_traces_to_npe_workload(
        args.input_filepath, args.output_filepath, args.quiet
    )
